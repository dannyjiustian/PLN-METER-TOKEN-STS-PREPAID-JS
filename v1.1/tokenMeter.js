...
class TokenDecryptor {
  ...
  extractTokenInfo(decryptedTokenBlock) {
    // Extract the token subclass from the first 4 bits
    console.log(
      `Token subclass binary: ${decryptedTokenBlock.substring(0, 4)}`
    );

    // Extract random bits (4-8) used for token identification or other purposes
    console.log(`Token rnd binary: ${decryptedTokenBlock.substring(4, 8)}`);

    // Extract the token issue date (minutes since base date) from bits 8-32
    const tidBinary = decryptedTokenBlock.substring(8, 32);
    const minutesSinceBaseDate = HelperFunction.binToDec(tidBinary);

    // Calculate and display the actual issue date based on a base date (2020-01-01)
    const baseDate = new Date("1993-01-01T00:00:00Z");
    const issueDate = new Date(
      baseDate.getTime() + minutesSinceBaseDate * 60000
    );
    console.log(`Token tid binary: ${tidBinary}`);
    console.log(
      `Token Date of Issue: ${issueDate
        .toISOString()
        .slice(0, 19)
        .replace("T", " ")}`
    );
