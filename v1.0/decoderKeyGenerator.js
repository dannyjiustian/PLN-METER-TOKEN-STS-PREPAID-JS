...
class DecoderKeyGenerator {
    constructor(keyType, supplyGroupCode, tariffIndex, keyRevisionNumber, decoderReferenceNumber) {
    ...
    buildPanBlock() {
        ...
        builder.push(this.decoderReferenceNumber.length === 11 ? this.IIN_2.substring(1) : this.IIN_1);
    ...
    async generateDecoderKey() {
        ...
        const encryptedResultXor = this.xor(panControlXorResult, CryptoJS.enc.Hex.parse(encryptionResult.toString(CryptoJS.enc.Hex)));

        // Generate the final 64-bit decoder key by XORing the vending key with the result
        this.decoderKeyHex = CryptoJS.enc.Hex.stringify(this.xor(vendingKey, encryptedResultXor)).toUpperCase(); // Convert to hex and store in uppercase
    }
}
...
const decoderKeyGen = new DecoderKeyGenerator(keyType, supplyGroupCode, tariffIndex, keyRevisionNumber, decoderReferenceNumber);
